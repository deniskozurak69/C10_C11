    ...
    const poll = async () => {
        try {
            ...
            if (response.ok) {
                ...
                pollInterval = Math.max(minInterval, pollInterval / 2);
                console.log('Message from server:', taskAction);
            } else {
                
                pollInterval = Math.min(maxInterval, pollInterval * idleMultiplier);
            }
        } catch (error) {
        ...
        console.log('Current polling interval:', pollInterval, 'ms');
